...
export interface DemoState {
  ...
  latencyBreakdown: {
    ...
    downSaved: number;
    protected: boolean;
    protectedDown: boolean;
    downTransport: number;
    playbackBuffer: number;
    upVariation: number;
    downVariation: number;
    baselineUpVariation: number;
    baselineDownVariation: number;
  };
  assuranceFlows: Array<{ id: string; label: string; status: 'active' | 'inactive' | 'unavailable' | 'standard' }>;
  futureOpen: boolean;
